...
const SurveyPage = () => {
  ...
  // --- 1. LOGIC: Define Steps ---
  // 'Intro' is index 0. Then A, B, C...
  const steps = ['Intro', 'A', 'B', 'C', 'D', 'E', 'F', 'G'];

  const currentKey = steps[activeStep];
  const isIntro = currentKey === 'Intro';

  // Helper to identify which Main Section a question belongs to
  const getSectionKey = (sectionString) => {
    if (sectionString.includes("Section A")) return 'A';
    if (sectionString.includes("Section B")) return 'B';
    if (sectionString.startsWith("C")) return 'C'; 
    if (sectionString.startsWith("D")) return 'D'; 
    if (sectionString.includes("Section E")) return 'E';
    if (sectionString.includes("Section F")) return 'F';
    if (sectionString.includes("Section G")) return 'G';
    return 'Other';
  };

  // Filter questions for the current page (only if not Intro)
  const pageQuestions = useMemo(() => {
    if (isIntro) return [];
    return questions.filter(q => getSectionKey(q.section) === currentKey);
  }, [currentKey, isIntro]);
  ...
  const getPageTitle = (key) => {
    const titles = {
      'A': 'Profile',
      'B': 'Exposure',
      'C': 'Perception of Frontline Staff',
      'D': 'Domain Specific Assessment',
      'E': 'Digital Governance & AI',
      'F': 'Role & Approach',
      'G': 'Qualitative Insights'
    };
    return titles[key] || '';
  };
    ...
    <>
      ...
      {!isIntro && (
        <Box sx={{ position: 'sticky', top: 64, zIndex: 10, bgcolor: '#f8fafc', pb: 2, borderBottom: '1px solid #e2e8f0' }}>
          ...
          />
          <Container maxWidth="md" sx={{ mt: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Chip label={`SECTION ${currentKey}`} color="primary" size="small" sx={{ fontWeight: 'bold' }} />
            <Typography variant="subtitle2" fontWeight="bold" color="#1e3a8a">
              {getPageTitle(currentKey)}
            </Typography>
          </Container>
        </Box>
      )}
          ...
          <Box>
            ...
            {isIntro && (
              <Paper elevation={4} sx={{ p: { xs: 3, md: 6 }, borderRadius: 4, textAlign: 'center', borderTop: '6px solid #1e3a8a' }}>
                ...
                <Typography variant="h4" fontWeight="800" color="#1e3a8a" gutterBottom sx={{ letterSpacing: '-0.5px' }}>
                  Feedback Form for IAS Officers
                </Typography>
                
                <Typography variant="h6" fontWeight="500" color="text.secondary" sx={{ mb: 4, lineHeight: 1.5 }}>
                ...
                <Box sx={{ textAlign: 'left', bgcolor: '#f1f5f9', p: 3, borderRadius: 2, mb: 5 }}>
                  ...
                  <Typography variant="body2" fontWeight="bold" color="#1e3a8a">
                    Your responses will remain anonymous and will be used for analysis and improvement of administrative practices.
                  </Typography>
                </Box>
                ...
                <Button 
                  ...
                  sx={{ px: 6, py: 2, fontSize: '1.2rem', borderRadius: 2, bgcolor: '#1e3a8a', '&:hover': { bgcolor: '#1e40af' } }}
                >
                  Start Survey
                </Button>
              </Paper>
            ...
            {!isIntro && (
              <>
                ...
                {pageQuestions.map((q, index) => {
                  const isNewSubSection = index === 0 || q.section !== pageQuestions[index - 1].section;
                      ...
                      {isNewSubSection && (
                        <Box sx={{ mt: 4, mb: 2 }}>
                           <Divider textAlign="left">
                             <Chip label={q.section} sx={{ fontWeight: 'bold', fontSize: '0.85rem' }} />
                           </Divider>
                        </Box>
                      )}

                      <Paper elevation={3} sx={{ p: 4, mb: 3, borderRadius: 3, position: 'relative' }}>
                                  ...
                                  return (
                                    <Box key={val} onClick={() => handleTextChange(q.id, val)}
                                      sx={{ 
                                        ...
                                        transform: isSelected ? 'scale(1.15)' : 'scale(1)', transition: 'all 0.2s'
                                      }}
                                    >
                                      <Box sx={{ 
                                        ...
                                        fontWeight: 'bold', fontSize: '1.1rem', boxShadow: isSelected ? 4 : 0
                                      }}>
                                        {val}
                                      </Box>
                                    </Box>
